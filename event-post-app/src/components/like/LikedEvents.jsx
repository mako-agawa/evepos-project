...
export default function LikedUsers() {
    ...
    // 修正: オプショナルチェイニングを使用
    const mmdd = getEventDate(events?.date);

    const weekday = getEventWeekday(events?.date);
    ...
    //     currentUser && event.liked?.some(liked => liked.user_id )
    // );

    return (
        <div className="flex flex-col pb-4 h-full mx-auto">
            <h1 className="text-gray-400 border-b-2 border-orange-300 px-6 text-xl font-semibold mb-6">Liked Events</h1>
            <div className="w-full">
                {events.length > 0 ? (
                    ...
                    <p className="text-center text-gray-500">いいねしたイベントはありません。</p>
                )}
            </div>
        </div>
    );
